...
RE_TIMER_KW = re.compile(r'timer|countdown|alarm')
RE_MUSIC_KW = re.compile(r'youtube|music|jazz|song|play|spotify')
RE_MUSIC_CLEAN = re.compile(r'(play|on youtube music|on youtube|on spotify)')
RE_WEATHER_KW = re.compile(r'weather|temperature|forecast|rain|sunny')
RE_WEATHER_LOC = re.compile(r'(?:in|at|for)\s+(.+)')
RE_SCREENSHOT_KW = re.compile(r'screenshot|screen\s*cap|capture\s*(the\s*)?screen|snap')
RE_CLIPBOARD_KW = re.compile(r'clipboard|paste|what.*copied|copy that')
RE_SYSINFO_KW = re.compile(r'battery|cpu|memory|ram|disk|system\s*info|storage')
RE_VOLUME_KW = re.compile(r'volume|mute|unmute|louder|quieter|sound')
RE_DIGITS = re.compile(r'(\d+)')
RE_MUTE = re.compile(r'mute')
RE_UNMUTE = re.compile(r'unmute')
RE_NOTE_KW = re.compile(r'(create|make|write|add)\s*(a\s*)?note')
RE_NOTE_CLEAN = re.compile(r'(create|make|write|add)\s*(a\s*)?note\s*(saying|that says|with)?\s*')
RE_REMINDER_KW = re.compile(r'remind|reminder')
RE_REMINDER_CLEAN = re.compile(r'(remind me to|set a reminder to|add a reminder to|remind me)\s*')
RE_FINDER_KW = re.compile(r'(open|show)\s*(the\s*)?(downloads?|documents?|desktop|home|finder|folder)')
RE_FINDER_MATCH = re.compile(r'(downloads?|documents?|desktop|home|finder)')
RE_MESSAGE_KW = re.compile(r'message')
RE_FOLLOWUP_KW = re.compile(r'follow-up|follow up|meeting|calendar')
RE_SEARCH_KW = re.compile(r'search|google|look up')
RE_SEARCH_CLEAN = re.compile(r'(search|the web|for|on google|on the internet|google|look up)')
RE_MAPS_KW = re.compile(r'map|direction|navigate|where is')
RE_MAPS_CLEAN = re.compile(r'(open maps? to|show me|navigate to|directions? to|where is)')
RE_SEND_MSG_KW = re.compile(r'send.*(message|text|imessage)')
RE_EVENT_KW = re.compile(r'(create|add|schedule|new).*(event|meeting|appointment)')
RE_OPEN_APP = re.compile(r'open\s+(\w+)')
RE_BATTERY_PCT = re.compile(r'(\d+)%')
RE_DISK_PCT = re.compile(r'(\d+)%')
RE_SPLIT_CMD = re.compile(r'\s+(?:and then|and also|and|then|also)\s+', re.IGNORECASE)
...
def classify(text: str, session_state: dict = None) -> dict:
    ...
    # Music
    if RE_MUSIC_KW.search(t):
        query = RE_MUSIC_CLEAN.sub('', t).strip()
    ...
    # Notes
    if RE_NOTE_KW.search(t):
        body = RE_NOTE_CLEAN.sub('', t).strip()
    ...
    # Reminders
    if RE_REMINDER_KW.search(t):
        body = RE_REMINDER_CLEAN.sub('', t).strip()
    ...
    # Finder
    if RE_FINDER_KW.search(t):
        m4 = RE_FINDER_MATCH.search(t)
    ...
    if RE_MESSAGE_KW.search(t) and RE_FOLLOWUP_KW.search(t):
    ...
    # Web search
    if RE_SEARCH_KW.search(t):
        query = RE_SEARCH_CLEAN.sub('', t).strip()
    ...
    # Maps
    if RE_MAPS_KW.search(t):
        query = RE_MAPS_CLEAN.sub('', t).strip()
...
def execute(plan: dict) -> dict:
    ...
    if intent == "SYSTEM_INFO":
        ...
        bat_match = RE_BATTERY_PCT.search(battery)
        ...
        disk = run_cmd(["df", "-h", "/"])
        disk_match = RE_DISK_PCT.search(disk)
