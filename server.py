...
async def run_osascript(script: str) -> str:
    # ⚡ Bolt: Using await run_cmd instead of synchronous subprocess.run to prevent blocking the async event loop
    try:
        res = await run_cmd(["osascript", "-e", script], timeout=10)
        return res
...
async def execute(plan: dict) -> dict:
    ...
    if intent == "GET_WEATHER":
        ...
        try:
            res = await run_cmd(["curl", "-s", f"https://wttr.in/{loc_param}?format=3"], timeout=5)
            weather = res.strip() if res and res != "ok" else "Weather data unavailable"
        except:
    ...
    if intent == "GET_IP":
        ...
        try:
            ...
            if my_ip == "127.0.0.1":
                # Try another way
                ip_out = await run_cmd(["hostname", "-I"])
                my_ip = ip_out.split()[0] if ip_out and ip_out != "ok" else "127.0.0.1"
        except:
            my_ip = "Unknown"
